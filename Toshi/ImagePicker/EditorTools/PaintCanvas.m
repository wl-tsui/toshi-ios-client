...
@interface PaintCanvas () <UIGestureRecognizerDelegate>
{
    PaintBuffers *_buffers;
    CGFloat _screenScale;
    CGAffineTransform _canvasTransform;
    CGRect _dirtyRect;
    
    PaintInput *_input;
    PaintPanGestureRecognizer *_gestureRecognizer;
    bool _beganDrawing;
...
- (void)setPainting:(Painting *)painting
{
    ...
    __weak PaintCanvas *weakSelf = self;
    painting.contentChanged = ^(CGRect rect)
    {
        __strong PaintCanvas *strongSelf = weakSelf;
        if (strongSelf == nil)
            return;
        
        if (!CGRectEqualToRect(rect, CGRectZero))
            strongSelf->_dirtyRect = PaintUnionRect(strongSelf->_dirtyRect, rect);
        else
...
{
    _input = [[PaintInput alloc] init];
    
    _gestureRecognizer = [[PaintPanGestureRecognizer alloc] initWithTarget:self action:@selector(handlePan:)];
...
- (void)handlePan:(PaintPanGestureRecognizer *)gestureRecognizer
{
...
- (void)setContext:(EAGLContext *)context
{
    ...
    if (context != nil)
    {
        _buffers = [PaintBuffers buffersWithGLContext:context layer:(CAEAGLLayer *)self.layer];
        [_buffers update];
    }
}
