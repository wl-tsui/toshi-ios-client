...
const CGFloat PaintRadialBrushHardness = 0.92f;

@implementation PaintRadialBrush
...
- (CGImageRef)stampRef
{
    ...
    if (image == NULL)
        image = [self generateRadialStampForSize:PaintBrushTextureSize hardness:PaintRadialBrushHardness];
...
- (CGImageRef)previewStampRef
{
    if (_previewStampRef == NULL)
        _previewStampRef = [self generateRadialStampForSize:PaintBrushPreviewTextureSize hardness:PaintRadialBrushHardness];
