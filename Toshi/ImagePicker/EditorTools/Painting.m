...
@interface Painting ()
{
    ...
    __weak PaintUndoManager *_undoManager;
...
- (void)paintStroke:(PaintPath *)path clearBuffer:(bool)clearBuffer completion:(void (^)(void))completion
{
    [self performAsynchronouslyInContext:^
    {
        ...
        if (status == GL_FRAMEBUFFER_COMPLETE)
        {
            ...
            [self _setupBrush];
            bounds = [PaintRender renderPath:path renderState:_renderState];
        }
        ...
        _activeStrokeBounds = PaintUnionRect(_activeStrokeBounds, bounds);
...
- (void)commitStrokeWithColor:(UIColor *)color erase:(bool)erase
{
    [self performAsynchronouslyInContext:^
    {
        ...
        [self updateWithBlock:^
        {
            ...
            PaintShader *shader = erase ? [self shaderForKey:@"compositeWithEraseMask"] : [self shaderForKey:@"compositeWithMask"];
...
- (void)_setupBrush
{
    PaintShader *shader = [self shaderForKey:_brush.lightSaber ? @"brushLight" : @"brush"];
    glUseProgram(shader.program);
    
    glActiveTexture(GL_TEXTURE0);
    
    if (_brushTexture == nil)
        _brushTexture = [[PaintTexture alloc] initWithCGImage:_brush.stampRef forceRGB:false];
...
- (void)renderWithProjection:(GLfloat *)projection
{
    if (_activePath != nil)
    {
        if (_activePath.action == PaintActionErase)
...
{
    PaintShader *shader = erase ? [self shaderForKey:@"blitWithEraseMask"] : [self shaderForKey:@"blitWithMask"];
    if (_brush.lightSaber)
...
{
    PaintShader *shader = [self shaderForKey:@"blit"];
    glUseProgram(shader.program);
...
- (NSData *)imageDataForRect:(CGRect)rect resultPaintingData:(NSData **)resultPaintingData
{
    ...
    PaintShader *blitShader = [self shaderForKey:@"nonPremultipliedBlit"];
...
- (void)registerUndoInRect:(CGRect)rect
{
    ...
    PaintSlice *slice = [[PaintSlice alloc] initWithData:paintingData bounds:rect];
    NSInteger uuid;
    arc4random_buf(&uuid, sizeof(NSInteger));
    [_undoManager registerUndoWithUUID:uuid block:^(Painting *painting, __unused PhotoEntitiesContainerView *entitiesContainer, __unused NSInteger uuid)
    {
...
- (void)restoreSlice:(PaintSlice *)slice redo:(bool)redo
{
    [self performAsynchronouslyInContext:^
    {
        if (!redo)
