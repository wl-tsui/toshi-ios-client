...
const NSInteger PaintRenderStateDefaultSize = 256;

@interface PaintRenderState ()
{
...
@implementation PaintRenderState
...
- (void)prepare
{
    ...
    _count = 0;
    _allocatedCount = PaintRenderStateDefaultSize;
    _values = malloc(sizeof(CGFloat) * PaintRenderStateDefaultSize * 5);
}
...
- (void)appendValuesCount:(NSUInteger)count
{
    ...
    if (newTotalCount > _allocatedCount || _values == NULL)
    {
        ...
        NSInteger newCount = MAX(_allocatedCount * 2, PaintRenderStateDefaultSize);
...
+ (void)_paintStamp:(PaintPoint *)point state:(PaintRenderState *)state
{
...
+ (void)_paintFromPoint:(PaintPoint *)lastLocation toPoint:(PaintPoint *)location state:(PaintRenderState *)state
{
    CGFloat f, distance = PaintDistance(lastLocation.CGPoint, location.CGPoint);
    CGPoint vector = PaintSubtractPoints(location.CGPoint, lastLocation.CGPoint);
    ...
    if (distance > 0.0f)
        unitVector = PaintMultiplyPoint(vector, 1.0f / distance);
    
    CGPoint start = PaintAddPoints(lastLocation.CGPoint, PaintMultiplyPoint(unitVector, state.remainder));
    ...
    for (f = state.remainder; f <= distance; f += step)
    {
        ...
        start = PaintAddPoints(start, PaintMultiplyPoint(unitVector, step));
...
+ (CGRect)_drawWithState:(PaintRenderState *)state
{
    ...
    for (NSUInteger i = 0; i < state.count; i++)
    {
        ...
        CGPoint center = PaintCenterOfRect(rect);
        ...
        dataBounds = PaintUnionRect(dataBounds, CGRectIntegral(boxBounds));
