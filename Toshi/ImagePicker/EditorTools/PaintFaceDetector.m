...
+ (instancetype)faceWithBounds:(CGRect)bounds angle:(CGFloat)angle leftEye:(PaintFaceEye *)leftEye rightEye:(PaintFaceEye *)rightEye mouth:(PaintFaceMouth *)mouth;

@end


@interface PaintFaceEye ()
...
@implementation PaintFaceDetector
...
+ (SSignal *)detectFacesInImage:(UIImage *)image originalSize:(CGSize)originalSize
{
    return [[SSignal alloc] initWithGenerator:^id<SDisposable>(SSubscriber *subscriber)
    {
        ...
        for (CIFaceFeature *feature in features)
        {
            ...
            PaintFace *face = [self _paintFaceForFaceFeature:feature initialSize:image.size targetSize:originalSize];
            if (face != nil)
...
+ (PaintFace *)_paintFaceForFaceFeature:(CIFaceFeature *)faceFeature initialSize:(CGSize)initialSize targetSize:(CGSize)targetSize
{
    if (faceFeature == nil)
        return nil;
    ...
    PaintFaceEye *leftEye = [PaintFaceEye eyeWithPosition:[self _transposedPoint:faceFeature.leftEyePosition initialSize:initialSize targetSize:targetSize] closed:faceFeature.leftEyeClosed];
    PaintFaceEye *rightEye = [PaintFaceEye eyeWithPosition:[self _transposedPoint:faceFeature.rightEyePosition initialSize:initialSize targetSize:targetSize] closed:faceFeature.rightEyeClosed];
    
    PaintFaceMouth *mouth = nil;
    
    if (faceFeature.hasMouthPosition)
        mouth = [PaintFaceMouth mouthWithPosition:[self _transposedPoint:faceFeature.mouthPosition initialSize:initialSize targetSize:targetSize] smiling:faceFeature.hasSmile];
    
    return [PaintFace faceWithBounds:[self _transposedRect:faceFeature.bounds initialSize:initialSize targetSize:targetSize] angle:[self _transposedAngle:TGDegreesToRadians(faceFeature.faceAngle)] leftEye:leftEye rightEye:rightEye mouth:mouth];
}
...
@implementation PaintFace

+ (instancetype)faceWithBounds:(CGRect)bounds angle:(CGFloat)angle leftEye:(PaintFaceEye *)leftEye rightEye:(PaintFaceEye *)rightEye mouth:(PaintFaceMouth *)mouth
{
    PaintFace *face = [[PaintFace alloc] init];
...
- (CGPoint)foreheadPoint
{
    ...
    CGPoint point = PaintCenterOfRect(_bounds);
...
@implementation PaintFaceFeature

@end


@implementation PaintFaceEye

+ (instancetype)eyeWithPosition:(CGPoint)position closed:(bool)closed
{
    PaintFaceEye *eye = [[PaintFaceEye alloc] init];
...
@implementation PaintFaceMouth

+ (instancetype)mouthWithPosition:(CGPoint)position smiling:(bool)smiling
{
    PaintFaceMouth *mouth = [[PaintFaceMouth alloc] init];
