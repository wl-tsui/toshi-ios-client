...
const CGFloat PhotoPaintColorWeightGestureRange = 320.0f;
const CGFloat PhotoPaintVerticalThreshold = 5.0f;
const CGFloat PhotoPaintPreviewOffset = -70.0f;
const CGFloat PhotoPaintPreviewScale = 2.0f;
const CGFloat PhotoPaintDefaultBrushWeight = 0.22f;
const CGFloat PhotoPaintDefaultColorLocation = 1.0f;

@interface PhotoPaintColorPickerKnobCircleView : UIView
{
...
@interface PhotoPaintColorPickerKnob : UIView
{
    UIView *_wrapperView;
    UIImageView *_shadowView;
    PhotoPaintColorPickerKnobCircleView *_backgroundView;
    PhotoPaintColorPickerKnobCircleView *_colorView;
...
@interface PhotoPaintColorPickerBackground : UIView
...
@interface PhotoPaintColorPicker () <UIGestureRecognizerDelegate>
{
    PhotoPaintColorPickerBackground *_backgroundView;
    PhotoPaintColorPickerKnob *_knobView;
...
@implementation PhotoPaintColorPicker
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self != nil)
    {
        _backgroundView = [[PhotoPaintColorPickerBackground alloc] initWithFrame:self.bounds];
        _backgroundView.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
        [self addSubview:_backgroundView];
        
        _knobView = [[PhotoPaintColorPickerKnob alloc] initWithFrame:CGRectMake(0.0f, 0.0f, 24.0f, 24.0f)];
        [_knobView setColor:[PhotoPaintColorPicker colorForLocation:0.0f]];
...
- (CGFloat)restoreLastColorLocation
{
    ...
    return PhotoPaintDefaultColorLocation;
}
...
- (PaintSwatch *)swatch
{
    return [PaintSwatch swatchWithColor:[PhotoPaintColorPicker colorForLocation:_location] colorLocation:_location brushWeight:_weight];
}
...
- (UIColor *)color
{
    return [PhotoPaintColorPicker colorForLocation:_location];
}
...
{
    _location = location;
    [_knobView setColor:[PhotoPaintColorPicker colorForLocation:_location]];
...
- (bool)_hasBeganChangingWeight:(CGPoint)location
{
    switch (self.orientation)
    {
        case UIInterfaceOrientationLandscapeLeft:
            if (location.x > self.frame.size.width + PhotoPaintVerticalThreshold)
                return true;
            
        case UIInterfaceOrientationLandscapeRight:
            if (location.x < -PhotoPaintVerticalThreshold)
                return true;
            
        default:
            if (location.y < -PhotoPaintVerticalThreshold)
                return true;
    }
...
- (CGFloat)_weightLocation:(CGPoint)location
{
    ...
    {
        case UIInterfaceOrientationLandscapeLeft:
            weightLocation = (location.x - self.frame.size.width - PhotoPaintVerticalThreshold) / PhotoPaintColorWeightGestureRange;
            break;
            
        case UIInterfaceOrientationLandscapeRight:
            weightLocation = ((location.x * -1) - PhotoPaintVerticalThreshold) / PhotoPaintColorWeightGestureRange;
            break;
            
        default:
            weightLocation = ((location.y * -1) - PhotoPaintVerticalThreshold) / PhotoPaintColorWeightGestureRange;
            break;
    }
...
{
    NSArray *locations = [PhotoPaintColorPickerBackground locations];
    NSArray *colors = [PhotoPaintColorPickerBackground colors];
...
@implementation PhotoPaintColorPickerKnobCircleView
...
const CGFloat PhotoPaintColorSmallCircle = 4.0f;
const CGFloat PhotoPaintColorLargeCircle = 20.0f;

@implementation PhotoPaintColorPickerKnob
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self != nil)
    {
        ...
        _backgroundView = [[PhotoPaintColorPickerKnobCircleView alloc] initWithFrame:self.bounds];
        _backgroundView.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
        _backgroundView.color = [UIColor whiteColor];
        [_wrapperView addSubview:_backgroundView];
        
        _colorView = [[PhotoPaintColorPickerKnobCircleView alloc] init];
        ...
        _colorView.center = PaintCenterOfRect(self.bounds);
...
{
    CGFloat result = PhotoPaintColorSmallCircle + (PhotoPaintColorLargeCircle - PhotoPaintColorSmallCircle) * size;
    result = zoomed ? TGRetinaFloor(result) * PhotoPaintPreviewScale : floor(result);
    return result;
}
...
- (void)updateLocationAnimated:(bool)animated updateColorSize:(bool)updateColorSize
{
    ...
    {
        CGPoint center = PaintCenterOfRect(self.bounds);
        CGFloat scale = 1.0f;
        if (_dragging)
        {
            scale = PhotoPaintPreviewScale;
         
            CGFloat offset = PhotoPaintPreviewOffset;
            if (self.changingWeight)
                offset -= _weight * PhotoPaintColorWeightGestureRange;
...
@implementation PhotoPaintColorPickerBackground
...
- (void)drawRect:(CGRect)rect
{
    ...
    CFArrayRef colors = (__bridge CFArrayRef)[PhotoPaintColorPickerBackground colors];
    CGFloat locations[[PhotoPaintColorPickerBackground colors].count];
    [PhotoPaintColorPickerBackground fillLocations:locations];
