...
NSString *const PhotoToolAncillaryShaderString = PGShaderString
(
...
@interface PhotoToolFilter : GPUImageFilter
{
...
@implementation PhotoToolFilter
...
@implementation PhotoToolComposer
{
...
{
    _imageSize = imageSize;
    [(PhotoToolFilter *)_filter setImageSize:imageSize];
}
...
- (void)compose
{
    [_tools sortUsingComparator:^NSComparisonResult(id obj1, id obj2) {
        PhotoTool *t1 = (PhotoTool *)obj1;
        PhotoTool *t2 = (PhotoTool *)obj2;
    ...
    for (PhotoTool *tool in _tools)
    {
        switch (tool.type)
        {
            case PhotoToolTypeShader:
            {
                if (tool.parameters)
                {
                    for (PhotoProcessPassParameter *parameter in tool.parameters)
                    {
    ...
    [shaderString appendString:PhotoToolAncillaryShaderString];
    ...
    PhotoToolFilter *filter = [[PhotoToolFilter alloc] initWithFragmentShaderFromString:shaderString];;
    for (PhotoProcessPassParameter *parameter in uniforms)
    {
