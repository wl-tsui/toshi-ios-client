...
@interface ImageMediaAttachment ()
{
    NSArray *_textCheckingResults;
}
@end

@implementation ImageMediaAttachment
...
- (id)init
{
    ...
    if (self != nil)
    {
        self.type = ImageMediaAttachmentType;
    }
    return self;
}
...
{
    ImageMediaAttachment *imageAttachment = [[ImageMediaAttachment alloc] init];
...
- (BOOL)isEqual:(id)object {
    if (![object isKindOfClass:[ImageMediaAttachment class]]) {
        return false;
    }
    ImageMediaAttachment *other = object;
...
{
    return [ImageMediaAttachment localImageIdForImageInfo:self.imageInfo];
}
...
- (MediaAttachment *)parseMediaAttachment:(NSInputStream *)is
{
    ...
    if (hasImageInfo != 0)
    {
        ImageInfo *imageInfo = [ImageInfo deserialize:is];
        if (imageInfo != nil)
