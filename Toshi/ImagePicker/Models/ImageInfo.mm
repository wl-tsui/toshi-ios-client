...
struct ImageSizeRecord
{
    CGSize size;
    NSString *url;
    int fileSize;
    
    ImageSizeRecord(CGSize size_, NSString *url_, int fileSize_) :
        size(size_), fileSize(fileSize_)
    {
        url = url_;
    }
    
    ImageSizeRecord(const ImageSizeRecord &other)
    {
        url = other.url;
        size = other.size;
    ...
    ImageSizeRecord & operator= (const ImageSizeRecord &other)
    {
        if (this != &other)
        {
            url = other.url;
...
@interface ImageInfo ()
{
    std::vector<ImageSizeRecord> sizes;
}
...
{
    if (![object isKindOfClass:[ImageInfo class]])
        return false;
...
{
    sizes.push_back(ImageSizeRecord(size, url, 0));
}

- (void)addImageWithSize:(CGSize)size url:(NSString *)url fileSize:(int)fileSize
{
    sizes.push_back(ImageSizeRecord(size, url, fileSize));
}
...
- (NSString *)closestImageUrlWithWidth:(int)width resultingSize:(CGSize *)resultingSize
{
    ...
    for (std::vector<ImageSizeRecord>::iterator it = sizes.begin(); it != sizes.end(); it++)
    {
        if (closestUrl == nil)
        {
            closestUrl = it->url;
