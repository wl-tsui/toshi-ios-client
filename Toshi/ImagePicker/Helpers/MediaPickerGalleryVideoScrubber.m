...
#import "MediaPickerGalleryVideoScrubberThumbnailView.h"
#import "MediaPickerGalleryVideoTrimView.h"
...
typedef enum
{
    MediaPickerGalleryVideoScrubberPivotSourceHandle,
    MediaPickerGalleryVideoScrubberPivotSourceTrimStart,
    MediaPickerGalleryVideoScrubberPivotSourceTrimEnd
} MediaPickerGalleryVideoScrubberPivotSource;

@interface MediaPickerGalleryVideoScrubber () <UIGestureRecognizerDelegate>
{
    ...
    MediaPickerGalleryVideoTrimView *_trimView;
    ...
    MediaPickerGalleryVideoScrubberPivotSource _pivotSource;
...
@implementation MediaPickerGalleryVideoScrubber
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self != nil)
    {
        ...
        _leftCurtainView.backgroundColor = [[PhotoEditorInterfaceAssets toolbarBackgroundColor] colorWithAlphaComponent:0.8f];
        [_wrapperView addSubview:_leftCurtainView];

        _rightCurtainView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 0, 0)];
        _rightCurtainView.backgroundColor = [[PhotoEditorInterfaceAssets toolbarBackgroundColor] colorWithAlphaComponent:0.8f];
        [_wrapperView addSubview:_rightCurtainView];
        
        __weak MediaPickerGalleryVideoScrubber *weakSelf = self;
        _trimView = [[MediaPickerGalleryVideoTrimView alloc] initWithFrame:CGRectZero];
        _trimView.exclusiveTouch = true;
        _trimView.trimmingEnabled = _allowsTrimming;
        _trimView.didBeginEditing = ^(bool start)
        {
            __strong MediaPickerGalleryVideoScrubber *strongSelf = weakSelf;
            if (strongSelf == nil)
                return;
            
            id<MediaPickerGalleryVideoScrubberDelegate> delegate = strongSelf.delegate;
            ...
            {
                if (start)
                    strongSelf->_pivotSource = MediaPickerGalleryVideoScrubberPivotSourceTrimStart;
                else
                    strongSelf->_pivotSource = MediaPickerGalleryVideoScrubberPivotSourceTrimEnd;
...
- (void)zoomIn
{
    ...
    switch (_pivotSource)
    {
        case MediaPickerGalleryVideoScrubberPivotSourceTrimStart:
            _zoomPivotCenter = [self _zoomPivotCenterForTrimStart];
            _zoomPivotPosition = trimStartValue;
            break;
            
        case MediaPickerGalleryVideoScrubberPivotSourceTrimEnd:
    ...
    id<MediaPickerGalleryVideoScrubberDataSource> dataSource = self.dataSource;
...
- (void)cancelZoomIn
{
    ...
    id<MediaPickerGalleryVideoScrubberDelegate> delegate = self.delegate;
...
{
    MediaPickerGalleryVideoScrubberThumbnailView *thumbnailView = [[MediaPickerGalleryVideoScrubberThumbnailView alloc] initWithImage:image originalSize:_originalSize cropRect:_cropRect cropOrientation:_cropOrientation cropMirrored:_cropMirrored];
...
{
    _currentTimeLabel.text = [MediaPickerGalleryVideoScrubber _stringFromTotalSeconds:(NSInteger)self.value];
    _inverseTimeLabel.text = [NSString stringWithFormat:@"-%@", [MediaPickerGalleryVideoScrubber _stringFromTotalSeconds:(NSInteger)self.duration - (NSInteger)self.value]];
}
...
- (void)handlePress:(UILongPressGestureRecognizer *)gestureRecognizer
{
    switch (gestureRecognizer.state)
    {
        case UIGestureRecognizerStateBegan:
        {
            ...
            if ([self zoomAvailable])
            {
                _pivotSource = MediaPickerGalleryVideoScrubberPivotSourceHandle;
