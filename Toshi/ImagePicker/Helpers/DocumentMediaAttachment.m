...
@interface DocumentMediaAttachment () {
    NSArray *_textCheckingResults;
}

@end

@implementation DocumentMediaAttachment
...
- (id)init
{
    ...
    if (self != nil)
    {
        self.type = DocumentMediaAttachmentType;
    }
    return self;
}

- (instancetype)copyWithZone:(NSZone *)__unused zone {
    DocumentMediaAttachment *document = [[DocumentMediaAttachment alloc] init];
...
- (MediaAttachment *)parseMediaAttachment:(NSInputStream *)is
{
    ...
    DocumentMediaAttachment *documentAttachment = [[DocumentMediaAttachment alloc] init];
    ...
    if (thumbnailExists)
    {
        documentAttachment.thumbnailInfo = [ImageInfo deserialize:is];
    }
    ...
    {
        if ([attribute isKindOfClass:[DocumentAttributeFilename class]])
        {
    ...
    if (!hasFilenameAttribute)
    {
        ...
        [array addObject:[[DocumentAttributeFilename alloc] initWithFilename:legacyFileName]];
...
- (NSString *)fileName
{
    ...
    for (id attribute in _attributes)
    {
        if ([attribute isKindOfClass:[DocumentAttributeFilename class]])
        {
            fileName = ((DocumentAttributeFilename *)attribute).filename;
            break;
        }
    }
...
{
    return [DocumentMediaAttachment safeFileNameForFileName:[self fileName]];
}
...
- (bool)isAnimated {
    for (id attribute in _attributes) {
        if ([attribute isKindOfClass:[DocumentAttributeAnimated class]]) {
            return true;
        }
    }
...
//    for (id attribute in _attributes) {
//        if ([attribute isKindOfClass:[DocumentAttributeSticker class]]) {
//            return ((DocumentAttributeSticker *)attribute).packReference != nil;
//        }
//    }
//    
//    return false;
...
//    for (id attribute in _attributes) {
//        if ([attribute isKindOfClass:[DocumentAttributeSticker class]]) {
//            return ((DocumentAttributeSticker *)attribute).packReference;
//        }
//    }
//    return nil;
//}
...
- (bool)isVoice {
    for (id attribute in _attributes) {
        if ([attribute isKindOfClass:[DocumentAttributeAudio class]]) {
            DocumentAttributeAudio *audio = attribute;
            if (audio.isVoice) {
                return true;
            }
        }
...
- (CGSize)pictureSize {
    for (id attribute in _attributes) {
        if ([attribute isKindOfClass:[DocumentAttributeImageSize class]]) {
            return ((DocumentAttributeImageSize *)attribute).size;
        } else if ([attribute isKindOfClass:[DocumentAttributeVideo class]]) {
            return ((DocumentAttributeVideo *)attribute).size;
        }
    }
