...
//{
//    return ScaleAndRoundCornersWithOffset(image, size, CGPointZero, size, 0, nil, true, nil);
//}
...
{
    return ScaleAndRoundCornersWithOffset(image, size, CGPointZero, imageSize, radius, overlay, opaque, backgroundColor);
}
...
{
    return ScaleAndRoundCornersWithOffsetAndFlags(image, size, offset, imageSize, radius, overlay, opaque, backgroundColor, 0);
}
...
UIImage *ScaleAndRoundCornersWithOffsetAndFlags(UIImage *image, CGSize size, CGPoint offset, CGSize imageSize, int radius, UIImage *overlay, bool opaque, UIColor *backgroundColor, int flags)
{
    ...
    //if (flags & ScaleImageScaleSharper)
...
UIImage *ScaleAndBlurImage(NSData *data, __unused CGSize size, __autoreleasing NSData **blurredData)
{
    CFAbsoluteTime startTime = CFAbsoluteTimeGetCurrent();
    
    UIImage *image = [[UIImage alloc] initWithData:data];
    //image = ScaleImageToPixelSize(image, CGSizeMake(128, 128));
...
UIImage *ModernAttachmentImage(UIImage *source, CGSize sourceSize, CGSize targetSize)
{
    static UIImage *borderImage = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^
...
UIImage *AttachmentImage(UIImage *source, CGSize sourceSize, CGSize size, __unused bool incoming, bool location)
{
    return ModernAttachmentImage(source, sourceSize, size);
    ...
    if (location)
    {
        ...
        [markerImage drawInRect:CGRectMake(floor((size.width - imageSize.width) / 2) - 4 * scale, floor((size.height - imageSize.height) / 2) - 5 * scale, imageSize.width * scale, imageSize.height * scale)];
    }
...
{
    return TGIsRetina() ? (floor(value * 2.0f)) / 2.0f : floor(value);
}
