...
@interface CameraCaptureSession () <AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate>
{
...
- (void)performInitialConfigurationWithCompletion:(void (^)(void))completion
{
    ...
    AVCaptureDevice *targetDevice = [CameraCaptureSession _deviceWithCameraPosition:_preferredCameraPosition];
    if (targetDevice == nil)
    ...
    if (_videoDevice != nil)
    {
        _preferredCameraPosition = [CameraCaptureSession _cameraPositionForDevicePosition:_videoDevice.position];
...
- (void)requestAudioSession
{
    ...
    {
        [_currentAudioSession setDisposable:[[AudioSessionManager instance] requestSessionWithType:AudioSessionTypePlayAndRecord interrupted:nil]];
    }];
}
...
- (void)setZoomLevel:(CGFloat)zoomLevel
{
    ...
    __weak CameraCaptureSession *weakSelf = self;
    [self _reconfigureDevice:self.videoDevice withBlock:^(AVCaptureDevice *device)
    {
        __strong CameraCaptureSession *strongSelf = weakSelf;
        if (strongSelf == nil)
            return;
...
{
    return [CameraCaptureSession _isZoomAvailableForDevice:self.videoDevice];
}
...
- (void)setCurrentFlashMode:(PGCameraFlashMode)mode
{
    [self _reconfigureDevice:self.videoDevice withBlock:^(AVCaptureDevice *device)
    {
        switch (self.currentMode)
        {
            ...
            case PGCameraModeClip:
            {
                AVCaptureTorchMode torchMode = [CameraCaptureSession _deviceTorchModeForCameraFlashMode:mode];
                if (device.hasTorch && [device isTorchModeSupported:torchMode])
                {
                    ...
                    else
                    {
                        ...
                        AVCaptureFlashMode flashMode = [CameraCaptureSession _deviceFlashModeForCameraFlashMode:mode];
...
{
    if (_videoDevice != nil)
        return [CameraCaptureSession _cameraPositionForDevicePosition:_videoDevice.position];
...
- (void)setCurrentCameraPosition:(PGCameraPosition)position
{
    ...
    AVCaptureDevice *deviceForTargetPosition = [CameraCaptureSession _deviceWithCameraPosition:position];
...
- (void)startVideoRecordingWithOrientation:(AVCaptureVideoOrientation)orientation mirrored:(bool)mirrored completion:(void (^)(NSURL *outputURL, CGAffineTransform transform, CGSize dimensions, NSTimeInterval duration, bool success))completion
{
    ...
    if (_videoFlashMode == PGCameraFlashModeAuto)
    {
        [self _reconfigureDevice:self.videoDevice withBlock:^(AVCaptureDevice *device)
        {
            AVCaptureTorchMode torchMode = [CameraCaptureSession _deviceTorchModeForCameraFlashMode:PGCameraFlashModeAuto];
            if (device.hasTorch && [device isTorchModeSupported:torchMode])
                device.torchMode = torchMode;
        }];
    }
    ...
    _movieWriter = [[CameraMovieWriter alloc] initWithVideoTransform:TGTransformForVideoOrientation(orientation, mirrored) videoOutputSettings:videoSettings audioOutputSettings:audioSettings];
