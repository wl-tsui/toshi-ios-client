...
@interface ModernButtonView ()
{
    ...
    ModernClockProgressView *_activityIndicator;
...
@implementation ModernButtonView
...
- (NSString *)viewStateIdentifier
{
    ...
    return [[NSString alloc] initWithFormat:@"ModernButtonView/%lx/%lx/%@/%lx/%lx", _backgroundImageFingerprint, _highightedBackgroundImageFingerprint, _title, (long)_titleFontFingerprint, (long)_imageFingerprint];
}
...
- (void)setDisplayProgress:(bool)displayProgress animated:(bool)animated {
    if (displayProgress) {
        if (_activityIndicator == nil) {
            _activityIndicator = [[ModernClockProgressView alloc] initWithFrame:CGRectMake(self.frame.size.width - 15.0f - 2.0f, self.frame.size.height - 15.0f - 2.0f, 15.0f, 15.0f)];
            [ModernClockProgressViewModel setupView:_activityIndicator forType:ModernClockProgressTypeOutgoingMediaClock];
