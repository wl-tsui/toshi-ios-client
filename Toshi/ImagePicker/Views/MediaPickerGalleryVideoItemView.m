...
- (void)prepareForRecycle
{
    ...
    _videoDurationVar = [SSignal single:nil];
...
- (void)setItem:(MediaPickerGalleryVideoItem *)item synchronously:(bool)synchronously
{
    ...
    __weak MediaPickerGalleryVideoItemView *weakSelf = self;
    _videoDurationVar = [[[item.durationSignal deliverOn:[SQueue mainQueue]] catch:^SSignal *(__unused id error)
    {
        __strong MediaPickerGalleryVideoItemView *strongSelf = weakSelf;
...
- (void)setIsCurrent:(bool)isCurrent
{
    ...
    __weak MediaPickerGalleryVideoItemView *weakSelf = self;
    [_videoDurationDisposable setDisposable:[[_videoDurationVar deliverOn:[SQueue mainQueue]] startWithNext:^(NSNumber *next)
    {
        __strong MediaPickerGalleryVideoItemView *strongSelf = weakSelf;
...
- (void)preparePlayerAndPlay:(bool)play
{
    ...
    [_playerItemDisposable setDisposable:[[itemSignal deliverOn:[SQueue mainQueue]] startWithNext:^(AVPlayerItem *playerItem)
    {
        ...
        strongSelf->_positionTimer = [TimerTarget scheduledMainThreadTimerWithTarget:self action:@selector(positionTimerEvent) interval:0.25 repeat:true];
