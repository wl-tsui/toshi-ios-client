...
@interface PhotoCropScrollView () <UIGestureRecognizerDelegate>
{
    UIView *_wrapperView;
...
@implementation PhotoCropScrollView
...
- (void)fitContentInsideBoundsAllowScale:(bool)allowScale maximize:(bool)maximize animated:(bool)animated completion:(void (^)(void))completion
{
    ...
    PhotoCropRectangle r2 = [self applyTransform:CGAffineTransformTranslate(currentTransform,
    ...
    PhotoCropRectangle r3 = [self applyTransform:t toRectangle:r2];
    ...
    if (!CGRectContainsRect(contentRect, boundsRect))
    {
        if (allowScale && (boundsRect.size.width > contentRect.size.width || boundsRect.size.height > contentRect.size.height))
            fitScaleBlock(boundsRect.size.width / ScaleToSize(boundsRect.size, contentRect.size).width);
    ...
    else
    {
        ...
        {
            CGFloat ratio = boundsRect.size.width / ScaleToSize(boundsRect.size, contentRect.size).width;
...
- (PhotoCropRectangle)rectangleFromCGRect:(CGRect)rect
{
    return (PhotoCropRectangle)
    {
...
- (CGRect)CGRectFromRectangle:(PhotoCropRectangle)rect
{
    return (CGRect)
    {
        .origin = rect.tl,
...
- (PhotoCropRectangle)applyTransform:(CGAffineTransform)transform toRect:(CGRect)rect
{
    CGAffineTransform t = CGAffineTransformMakeTranslation(CGRectGetMidX(rect), CGRectGetMidY(rect));
    t = CGAffineTransformConcat(transform, t);
    t = CGAffineTransformTranslate(t, -CGRectGetMidX(rect), -CGRectGetMidY(rect));
    
    PhotoCropRectangle r = [self rectangleFromCGRect:rect];
    return (PhotoCropRectangle)
    {
...
- (PhotoCropRectangle)applyTransform:(CGAffineTransform)t toRectangle:(PhotoCropRectangle)r
{
    return (PhotoCropRectangle)
    {
        .tl = CGPointApplyAffineTransform(r.tl, t),
