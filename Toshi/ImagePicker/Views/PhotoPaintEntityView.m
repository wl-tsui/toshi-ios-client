...
const CGFloat PhotoPaintEntityMinScale = 0.12f;

@interface PhotoPaintEntityView () <UIGestureRecognizerDelegate>
{
...
@implementation PhotoPaintEntityView
...
- (void)pan:(CGPoint)point absolute:(bool)absolute
{
    if (absolute)
        ...
        self.center = PaintAddPoints(self.center, point);
...
- (void)scale:(CGFloat)scale absolute:(bool)__unused absolute
{
    ...
    if (newScale < PhotoPaintEntityMinScale)
        scale = self.scale / PhotoPaintEntityMinScale;
...
@implementation PhotoPaintEntitySelectionView

- (void)update
{
    PhotoPaintEntityView *entityView = self.entityView;
    
    [entityView _pushIdentityTransformForMeasurement];
    self.transform = CGAffineTransformIdentity;
    CGRect bounds = entityView.selectionBounds;
    CGPoint center = PaintCenterOfRect(bounds);
