...
const CGFloat PhotoTextSelectionViewHandleSide = 30.0f;

@interface PhotoTextView ()

@end

@interface PhotoTextSelectionView () <UIGestureRecognizerDelegate>
{
    UIView *_leftHandle;
    UIView *_rightHandle;
...
@interface PhotoTextLayoutManager : NSLayoutManager
...
@interface PhotoTextEntityView () <UITextViewDelegate>
{
    PaintSwatch *_swatch;
    PhotoPaintFont *_font;
    CGFloat _baseFontSize;
    CGFloat _maxWidth;
    bool _stroke;
    
    PhotoTextView *_textView;
}
@end

@implementation PhotoTextEntityView
...
- (instancetype)initWithEntity:(PhotoPaintTextEntity *)entity
{
    ...
    if (self != nil)
    {
        ...
        _textView = [[PhotoTextView alloc] initWithFrame:CGRectZero];
...
- (PhotoPaintTextEntity *)entity
{
    PhotoPaintTextEntity *entity = [[PhotoPaintTextEntity alloc] initWithText:_textView.text font:_font swatch:_swatch baseFontSize:_baseFontSize maxWidth:_maxWidth stroke:_stroke];
...
{
    PhotoTextSelectionView *view = [[PhotoTextSelectionView alloc] init];
...
@implementation PhotoTextSelectionView
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self != nil)
    {
        ...
        _leftHandle = [[UIView alloc] initWithFrame:CGRectMake(0, 0, PhotoTextSelectionViewHandleSide, PhotoTextSelectionViewHandleSide)];
        ...
        _rightHandle = [[UIView alloc] initWithFrame:CGRectMake(0, 0, PhotoTextSelectionViewHandleSide, PhotoTextSelectionViewHandleSide)];
...
- (void)drawRect:(CGRect)rect
{
    ...
    CGPoint centerPoint = PaintCenterOfRect(rect);
...
@implementation PhotoTextView

- (instancetype)initWithFrame:(CGRect)frame
{
    NSTextStorage *textStorage = [[NSTextStorage alloc] init];
    PhotoTextLayoutManager *layoutManager = [[PhotoTextLayoutManager alloc] init];
...
- (CGFloat)strokeWidth
{
    return ((PhotoTextLayoutManager *)self.layoutManager).strokeWidth;
}

- (void)setStrokeWidth:(CGFloat)strokeWidth
{
    [(PhotoTextLayoutManager *)self.layoutManager setStrokeWidth:strokeWidth];
    [self setNeedsDisplay];
}

- (UIColor *)strokeColor
{
    return ((PhotoTextLayoutManager *)self.layoutManager).strokeColor;
}

- (void)setStrokeColor:(UIColor *)strokeColor
{
    [(PhotoTextLayoutManager *)self.layoutManager setStrokeColor:strokeColor];
    [self setNeedsDisplay];
}

- (CGPoint)strokeOffset
{
    return ((PhotoTextLayoutManager *)self.layoutManager).strokeOffset;
}

- (void)setStrokeOffset:(CGPoint)strokeOffset
{
    [(PhotoTextLayoutManager *)self.layoutManager setStrokeOffset:strokeOffset];
    [self setNeedsDisplay];
}
...
@implementation PhotoTextLayoutManager
