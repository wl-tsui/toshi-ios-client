...
const CGPoint PhotoEditorEdgeScrollTriggerOffset = { 100, 150 };

@interface PhotoEditorCollectionView () <UICollectionViewDelegateFlowLayout>
{
...
- (instancetype)initWithOrientation:(UIInterfaceOrientation)orientation cellWidth:(CGFloat)cellWidth
{
    ...
    if (self != nil)
    {
        ...
        [self registerClass:[PhotoFilterCell class] forCellWithReuseIdentifier:PhotoFilterCellKind];
        [self registerClass:[PhotoToolCell class] forCellWithReuseIdentifier:PhotoToolCellKind];
    }
    return self;
}
...
- (void)setSelectedItemIndexPath:(NSIndexPath *)indexPath
{
    ...
    for (NSIndexPath *i in visibleItemsIndexPathes)
    {
        ...
        if ([cell isKindOfClass:[PhotoFilterCell class]])
            [(PhotoFilterCell *)cell setFilterSelected:[i isEqual:indexPath]];
    }
}
...
{
    id <PhotoEditorCollectionViewFiltersDataSource> filtersDataSource = self.filtersDataSource;
    id <PhotoEditorCollectionViewToolsDataSource> toolsDataSource = self.toolsDataSource;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)__unused collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        PhotoFilter *filter = [filtersDataSource collectionView:self filterAtIndex:indexPath.row];
        
        cell = [self dequeueReusableCellWithReuseIdentifier:PhotoFilterCellKind forIndexPath:indexPath];
        [(PhotoFilterCell *)cell setPhotoFilter:filter];
        [(PhotoFilterCell *)cell setFilterSelected:[_selectedItemIndexPath isEqual:indexPath]];
        ...
        [filtersDataSource collectionView:self requestThumbnailImageForFilterAtIndex:indexPath.row completion:^(UIImage *thumbnailImage, bool cached, __unused bool finished)
        {
            ...
            {
                if ([[(PhotoFilterCell *)cell filterIdentifier] isEqualToString:filter.identifier])
                    [(PhotoFilterCell *)cell setImage:thumbnailImage animated:!cached];
            });
        }];
    }
    else if ([toolsDataSource respondsToSelector:@selector(collectionView:toolAtIndex:)])
    {
        cell = [self dequeueReusableCellWithReuseIdentifier:PhotoToolCellKind forIndexPath:indexPath];
        [(PhotoToolCell *)cell setPhotoTool:[toolsDataSource collectionView:self toolAtIndex:indexPath.row]];
    }
...
- (void)collectionView:(UICollectionView *)__unused collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([filtersDataSource respondsToSelector:@selector(collectionView:didSelectFilterWithIndex:)])
    {
        ...
        if (!vertical)
        {
            ...
            triggerOffset = PhotoEditorEdgeScrollTriggerOffset.x;
        ...
        else
        {
            ...
            triggerOffset = PhotoEditorEdgeScrollTriggerOffset.y;
            targetOverlap = itemSize + 2 * ((UICollectionViewFlowLayout *)self.collectionViewLayout).minimumLineSpacing;
        }
