#import "MessageImageViewTimestampView.h"
...
@interface MessageImageViewTimestampLayer : CALayer

@end

@implementation MessageImageViewTimestampLayer
...
@interface MessageImageViewTimestampView ()
{
    StaticBackdropAreaData *_backdropArea;
...
@implementation MessageImageViewTimestampView

/*+ (Class)layerClass
{
    return [MessageImageViewTimestampLayer class];
}*/
...
- (void)setBackdropArea:(StaticBackdropAreaData *)backdropArea transitionDuration:(NSTimeInterval)transitionDuration
{
    if (false && cpuCoreCount() >= 2 && transitionDuration > DBL_EPSILON)
    {
        ...
        if (currentContents != nil)
        {
            ...
            AnimationBlockDelegate *animationDelegate = [[AnimationBlockDelegate alloc] initWithLayer:transitionLayer];;
...
- (void)setTimestampString:(NSString *)timestampString signatureString:(NSString *)signatureString displayCheckmarks:(bool)displayCheckmarks checkmarkValue:(int)checkmarkValue displayViews:(bool)displayViews viewsValue:(int)viewsValue animated:(bool)animated
{
    ...
    if (displayViews) {
        viewsString = [MessageImageViewTimestampView stringForCount:viewsValue];
    }
    
    if (!StringCompare(_viewsString, viewsString)) {
    ...
    if (!StringCompare(_timestampString, timestampString) || !StringCompare(_signatureString, signatureString) || _displayCheckmarks != displayCheckmarks)
    {
