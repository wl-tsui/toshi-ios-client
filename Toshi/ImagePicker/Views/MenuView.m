...
@property (nonatomic, weak) id<MenuButtonViewDelegate> delegate;
...
- (void)setHighlighted:(BOOL)highlighted
{
    ...
    __strong id<MenuButtonViewDelegate> delegate = _delegate;
...
@interface MenuView () <MenuButtonViewDelegate, UIScrollViewDelegate>
{
...
- (void)setButtonsAndActions:(NSArray *)buttonsAndActions watcherHandle:(ASHandle *)watcherHandle
{
    ...
    {
        MenuButtonView *buttonView = [_buttonViews lastObject];
    ...
    index = -1;
    for (MenuButtonView *buttonView in _buttonViews)
    {
        index++;
...
- (void)setButtonHighlightDisabled:(bool)buttonHighlightDisabled
{
    ...
    for (MenuButtonView *view in _buttonViews)
...
- (void)sizeToFit
{
    ...
    if (currentPageButtons.count != 0) {
        if (currentPageButtons.count == 1) {
            for (MenuButtonView *buttonView in optionalButtons)
            {
                CGFloat buttonWidth = buttonView.frame.size.width;
                if (currentPageWidth + buttonWidth > maxWidth) {
                    break;
    ...
    for (NSMutableArray *page in pages) {
        NSUInteger index = 0;
        for (MenuButtonView *button in page) {
    ...
    for (NSArray *buttons in pages) {
        ...
        for (UIView *button in buttons) {
            ...
            CGFloat buttonWidth = floor(button.frame.size.width * factor);
    ...
    CGFloat arrowX = floor(_arrowLocation - _arrowTopView.frame.size.width / 2);
...
- (void)showInView:(UIView *)view fromRect:(CGRect)rect animated:(bool)animated
{
    ...
    _arrowLocation = floor(rect.origin.x + rect.size.width / 2) - frame.origin.x;
...
- (void)buttonPressed:(MenuButtonView *)buttonView
{
    NSInteger index = -1;
    for (MenuButtonView *listButtonView in _buttonViews)
    {
        index++;
        ...
        if (listButtonView == buttonView)
        {
            ...
            if ([self.superview isKindOfClass:[MenuContainerView class]])
                [(MenuContainerView *)self.superview hideMenu];
