...
@interface ModernGalleryZoomableItemView () <UIScrollViewDelegate>

@property (nonatomic, strong) UIView *internalContainerView;

@end

@implementation ModernGalleryZoomableItemView
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self != nil)
    {
        ...
        _containerView = [[ModernGalleryImageItemContainerView alloc] initWithFrame:_internalContainerView.bounds];
        [_internalContainerView addSubview:_containerView];
        
        _scrollView = [[ModernGalleryZoomableScrollView alloc] initWithFrame:_containerView.bounds];
        ...
        __weak ModernGalleryZoomableItemView *weakSelf = self;
        
        _scrollView.singleTapped = ^
        {
            __strong ModernGalleryZoomableItemView *strongSelf = weakSelf;
        ...
        ((ModernGalleryImageItemContainerView *)_containerView).contentView = ^UIView *
        {
            __strong ModernGalleryZoomableItemView *strongSelf = weakSelf;
            if (strongSelf != nil)
...
{
    [Hacks setAnimationDurationFactor:0.6f];
    if (_scrollView.zoomScale <= _scrollView.normalZoomScale + FLT_EPSILON)
    {
