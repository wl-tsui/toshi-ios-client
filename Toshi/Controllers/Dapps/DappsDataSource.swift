...
struct DappsDataSourceSection {
    ...
    init(name: String?, categoryId: Int? = nil, items: [DappsDataSourceItem]) {
        self.name = name
        self.items = items
...
final class DappsDataSource {
    ...
    private func fetchFrontPage() {
        ...
        operation.addExecutionBlock { [weak self] in
            ...
            DirectoryAPIClient.shared.getDappsFrontPage(completion: { dappsFrontPage, error in
                ...
                // Fetched categories
                let fetchedDappsSections = frontPage.categories.flatMap({ category -> DappsDataSourceSection? in

                    guard !strongOperation.isCancelled else { return nil }

                    let items = category.dapps.map({ dapp -> DappsDataSourceItem in
                        return DappsDataSourceItem(type: .dappFront, dapp: dapp)
                    })

                    return DappsDataSourceSection(name: frontPage.categoriesInfo[category.categoryId], categoryId: category.categoryId, items: items)
                })
    ...
    @discardableResult func adjustToSearchText(_ searchText: String) -> [DappsDataSourceSection] {
        ...
        if !searchText.isEmpty {
            ...
            let section = DappsDataSourceSection(name: Localized("dapps-search-with-google-section-title"), items: [item])
            results.insert(section, at: 0)
        }
        ...
        if let possibleURLString = searchText.asPossibleURLString,
            ...
            let section = DappsDataSourceSection(name: Localized("dapps-open-url-section-title"), items: [item])
