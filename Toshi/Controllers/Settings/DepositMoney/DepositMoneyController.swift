...
class DepositMoneyController: UIViewController {
    ...
    convenience init(for username: String, name _: String) {
        ...
        guard let paymentAddress = Cereal.shared.paymentAddress else { return }

        title = Localized("deposit_money_title")

        items = [
            .header(Localized("deposit_money_header_text")),
            .bulletPoint(Localized("deposit_money_1_title"), String(format: Localized("deposit_money_1_text"), paymentAddress)),
