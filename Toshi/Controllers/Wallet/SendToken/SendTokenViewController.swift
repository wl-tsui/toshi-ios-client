...
final class SendTokenViewController: UIViewController {
    ...
    deinit {
        print("Send token view controller deinitialised")
    }
        ...
        if let intent = QRCodeIntent(result: result) {
            ...
            case .addressInput(let address):
                controller.dismiss(animated: true, completion: { [weak self] in
                    self?.configurator.destinationAddress = address
                })
            default:
                let alert = UIAlertController.dismissableAlert(title: Localized("wallet_not_an_address_message"))
