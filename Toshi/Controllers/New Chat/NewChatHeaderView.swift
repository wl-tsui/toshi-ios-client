...
final class NewChatHeaderView: UIView {
    ...
    private func configure(for type: NewChatViewControllerType, with delegate: NewChatAddGroupHeaderDelegate?, searchBar: UISearchBar?) {
        switch type {
        case .newChat:
            let addGroupHeader = NewChatAddGroupHeader(with: delegate)
