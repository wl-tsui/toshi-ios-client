...
- (BOOL)messageBubbleIsActionableAtIndexPath:(NSIndexPath *)indexPath
{
    id<JSQMessageData> messageItem = [self.collectionView.dataSource collectionView:self.collectionView
                                                      messageDataForItemAtIndexPath:indexPath];

    return [messageItem isActionable];
}
...
- (CGSize)sizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    BOOL isActionable = [self messageBubbleIsActionableAtIndexPath:indexPath];
    CGFloat bottomActionsHeight = isActionable ? 35.0 : 0.0;
    return CGSizeMake(self.itemWidth, ceilf(finalHeight) + bottomActionsHeight);
}
