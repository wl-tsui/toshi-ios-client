...
protocol PaymentRequestControllerDelegate {
    func paymentRequestControllerDidFinish(valueInWei: NSDecimalNumber?)
}

class PaymentRequestController: UIViewController {
    var delegate: PaymentRequestControllerDelegate?
    ...
    lazy var toolbar: UIToolbar = {
        ...
        view.items = [self.cancelBarButton, spacing, self.continueBarButton]

        return view
    }()

    lazy var continueBarButton: UIBarButtonItem = {
        let item = UIBarButtonItem(barButtonSystemItem: .done, target: self, action: #selector(PaymentRequestController.sendRequest))

        return item
    }()

    lazy var cancelBarButton: UIBarButtonItem = {
        let item = UIBarButtonItem(barButtonSystemItem: .cancel, target: self, action: #selector(PaymentRequestController.cancelRequest))
...
extension PaymentRequestController: UITextFieldDelegate {
...
extension PaymentRequestController: UIToolbarDelegate {
