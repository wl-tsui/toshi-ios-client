...
NSString *const MediaAssetsVideoCellKind = @"MediaAssetsVideoCellKind";

@interface MediaAssetsVideoCell ()
{
...
@implementation MediaAssetsVideoCell
...
- (void)setItem:(NSObject *)item signal:(SSignal *)signal
{
    ...
    SSignal *adjustmentsSignal = [self.editingContext adjustmentsSignalForItem:(id<MediaEditableItem>)self.item];
    
    __weak MediaAssetsVideoCell *weakSelf = self;
    [_adjustmentsDisposable setDisposable:[adjustmentsSignal startWithNext:^(VideoEditAdjustments *next)
    {
        __strong MediaAssetsVideoCell *strongSelf = weakSelf;
        if (strongSelf == nil)
            return;
...
- (UIImage *)transitionImage
{
    ...
    VideoEditAdjustments *adjustments = (VideoEditAdjustments *)[self.editingContext adjustmentsForItem:(id<MediaEditableItem>)self.item];
    if ([adjustments isKindOfClass:[VideoEditAdjustments class]])
    {
        ...
        [[self.editingContext thumbnailImageSignalForItem:(id<MediaEditableItem>)self.item withUpdates:false synchronous:true] startWithNext:^(UIImage *next)
        {
            editedImage = next;
        }];
    ...
    {
        CGContextConcatCTM(UIGraphicsGetCurrentContext(), VideoCropTransformForOrientation(cropOrientation, self.frame.size, false));
