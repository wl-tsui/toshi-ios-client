...
#import "PhotoCropScrollView.h"
#import "PhotoCropAreaView.h"
#import "PhotoCropRotationView.h"

#import "Common.h"

const CGFloat PhotoCropViewOverscreenSize = 1000;
...
@interface PhotoCropView () <UIScrollViewDelegate>
{
    ...
    PhotoCropAreaView *_areaView;
    CGRect _previousAreaFrame;
    
    PhotoCropScrollView *_scrollView;
    ...
    PhotoCropRotationView *_rotationView;
...
- (instancetype)initWithOriginalSize:(CGSize)originalSize hasArbitraryRotation:(bool)hasArbitraryRotation
{
    ...
    if (self != nil)
    {
        ...
        __weak PhotoCropView *weakSelf = self;
        ...
        _scrollView = [[PhotoCropScrollView alloc] initWithFrame:CGRectZero];
        _scrollView.shouldBeginChanging = ^bool
        {
            __strong PhotoCropView *strongSelf = weakSelf;
            if (strongSelf == nil)
                return true;
        ...
        _scrollView.didBeginChanging = ^
        {
            ...
            [strongSelf->_areaView setGridMode:PhotoCropViewGridModeMajor animated:true];
        ...
        _areaView = [[PhotoCropAreaView alloc] initWithFrame:self.bounds];
        ...
        {
            _rotationView = [[PhotoCropRotationView alloc] initWithFrame:CGRectZero];
            ...
            _rotationView.didBeginChanging = ^
            {
                ...
                [strongSelf resetBackdropViewsAnimated:true];
                [strongSelf cancelConfirmCountdown];
                [strongSelf->_areaView setGridMode:PhotoCropViewGridModeMinor animated:true];
            ...
            _rotationView.didEndChanging = ^
            {
                ...
                [strongSelf startConfirmCountdown];
                [strongSelf->_areaView setGridMode:PhotoCropViewGridModeNone animated:true];
...
- (CGRect)_cappedAreaViewRectWithRect:(CGRect)cropRect
{
    ...
    {
        cappedRect.origin.x = MIN(self.frame.size.width - PhotoCropCornerControlSize.width, cappedRect.origin.x);
        cappedRect.size.width = MAX(PhotoCropCornerControlSize.width, self.frame.size.width - cappedRect.origin.x);
        if (aspectRatio > 0)
    ...
    {
        cappedRect.origin.y = MIN(self.frame.size.height - PhotoCropCornerControlSize.height, cappedRect.origin.y);
        cappedRect.size.height = MAX(PhotoCropCornerControlSize.height, self.frame.size.height - cappedRect.origin.y);
        if (aspectRatio > 0)
...
- (void)startConfirmCountdown
{
    ...
    _confirmTimer = [[TGTimer alloc] initWithTimeout:1.0f repeat:false completion:^
    {
        __strong PhotoCropView *strongSelf = weakSelf;
        if (strongSelf == nil)
            return;
...
- (void)performConfirmAnimated:(bool)animated updateInterface:(bool)updateInterface
{
    ...
    [_areaView setGridMode:PhotoCropViewGridModeNone animated:animated];
...
{
    return PhotoEditorVideoCrop(_imageView.image, _paintingImage, self.cropOrientation, self.rotation, self.cropRect, self.mirrored, maxSize, _originalSize, true, true);
}
...
- (void)rotate90DegreesCCWAnimated:(bool)animated
{
    ...
    _areaWrapperView.backgroundColor = [PhotoEditorInterfaceAssets toolbarBackgroundColor];
...
- (void)_layoutOverlayViewsWithFrame:(CGRect)frame animated:(bool)animated
{
    ...
    CGRect topOverlayFrame = CGRectMake(0, -PhotoCropViewOverscreenSize, overlayWrapperFrame.size.width, PhotoCropViewOverscreenSize);
    CGRect leftOverlayFrame = CGRectMake(-PhotoCropViewOverscreenSize, -PhotoCropViewOverscreenSize, PhotoCropViewOverscreenSize, overlayWrapperFrame.size.height + 2 * PhotoCropViewOverscreenSize);
    CGRect rightOverlayFrame = CGRectMake(overlayWrapperFrame.size.width, -PhotoCropViewOverscreenSize, PhotoCropViewOverscreenSize, overlayWrapperFrame.size.height + 2 * PhotoCropViewOverscreenSize);
    CGRect bottomOverlayFrame = CGRectMake(0, overlayWrapperFrame.size.height, overlayWrapperFrame.size.width, PhotoCropViewOverscreenSize);
...
- (CGSize)_areaSizeForCropRect:(CGRect)cropRect orientation:(UIImageOrientation)orientation
{
    ...
    CGSize areaSize = ScaleToSize(rotatedSize, self.bounds.size);
