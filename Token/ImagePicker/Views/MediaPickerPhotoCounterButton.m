...
const CGFloat PhotoCounterButtonMaskFade = 18;

@interface MediaPickerPhotoCounterButton ()
{
    UIView *_wrapperView;
...
@implementation MediaPickerPhotoCounterButton
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self != nil)
    {
        ...
        _countLabel.font = [Font systemFontOfSize:16];
        _countLabel.text = [StringUtils stringWithLocalizedNumber:0];
        ...
        _crossIconView.image = [UIImage imageNamed:@"Close"];
        ...
        CGFloat maskFade = PhotoCounterButtonMaskFade / maskWidth;
        ...
        _processingLabel.font = [Font systemFontOfSize:16];
...
- (void)setSelectedCount:(NSInteger)count animated:(bool)animated
{
    ...
    _countLabel.text = [NSString stringWithFormat:@"%@", [StringUtils stringWithLocalizedNumber:count]];
...
- (void)setActiveNumber:(NSInteger)number animated:(bool)__unused animated
{
    ...
    _processingLabel.text = [NSString stringWithFormat:TGLocalized(@"Processing"), [NSString stringWithFormat:TGLocalized(@"Nof"), [StringUtils stringWithLocalizedNumber:number]]];
...
- (void)_layoutProcessingViewsWithWidth:(CGFloat)width
{
    ...
    if (![self _useRtlLayout])
    {
        _processingLabel.frame = CGRectMake(PhotoCounterButtonMaskFade, _processingLabel.frame.origin.y, _processingLabel.frame.size.width, _processingLabel.frame.size.height);
        _processingMaskView.frame = CGRectMake(_countLabel.frame.origin.x - width - 4.5f - PhotoCounterButtonMaskFade, _processingMaskView.frame.origin.y, _processingMaskView.frame.size.width, _processingMaskView.frame.size.height);
...
- (void)cancelledProcessingAnimated:(bool)animated completion:(void (^)(void))completion
{
    void (^changeBlock)(void) = ^
    {
        ...
        {
            _processingLabel.frame = CGRectMake(-_processingLabel.frame.size.width, _processingLabel.frame.origin.y, _processingLabel.frame.size.width, _processingLabel.frame.size.height);
            _processingMaskView.frame = CGRectMake(19 + PhotoCounterButtonMaskFade, _processingMaskView.frame.origin.y, _processingMaskView.frame.size.width, _processingMaskView.frame.size.height);
...
- (bool)_useRtlLayout
{
    return (TGIsRTL() && ![TGLocalized(@"Processing") isEqualToString:@"Processing %@"]);
}
