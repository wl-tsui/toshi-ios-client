...
@interface PhotoEditorTintToolView ()
{
    UIView *_buttonsWrapper;
    NSArray *_swatchViews;
    
    ModernButton *_shadowsButton;
    ModernButton *_highlightsButton;
...
@property (nonatomic, weak) TintTool *tintTool;

@end

@implementation PhotoEditorTintToolView
...
- (instancetype)initWithEditorItem:(id<PhotoEditorItem>)editorItem
{
    ...
    if (self != nil)
    {
        ...
        {
            PhotoEditorTintSwatchView *swatchView = [[PhotoEditorTintSwatchView alloc] initWithFrame:CGRectMake(0, 0, PhotoEditorTintSwatchSize, PhotoEditorTintSwatchSize)];
        ...
        if ([editorItem isKindOfClass:[TintTool class]])
        {
            TintTool *tintTool = (TintTool *)editorItem;
        ...
        _intensityTitleLabel.font = [PhotoEditorInterfaceAssets editorItemTitleFont];
        _intensityTitleLabel.text = [TGLocalized(@"TintIntensity") uppercaseString];
        _intensityTitleLabel.textAlignment = NSTextAlignmentCenter;
        _intensityTitleLabel.textColor = [PhotoEditorInterfaceAssets editorItemTitleColor];
        _intensityTitleLabel.userInteractionEnabled = false;
        _intensityTitleLabel.hidden = true;
        [self addSubview:_intensityTitleLabel];
        
        _shadowsButton = [[ModernButton alloc] initWithFrame:CGRectMake(0, 0, 120, 20)];
        _shadowsButton.selected = true;
        _shadowsButton.backgroundColor = [UIColor clearColor];
        _shadowsButton.titleLabel.font = [PhotoEditorInterfaceAssets editorItemTitleFont];
        [_shadowsButton setTitle:[TGLocalized(@"ShadowsTint") uppercaseString] forState:UIControlStateNormal];
        ...
        _highlightsButton = [[ModernButton alloc] initWithFrame:CGRectMake(0, 0, 120, 20)];
        _highlightsButton.backgroundColor = [UIColor clearColor];
        _highlightsButton.titleLabel.font = [PhotoEditorInterfaceAssets editorItemTitleFont];
        [_highlightsButton setTitle:[TGLocalized(@"HighlightsTint") uppercaseString] forState:UIControlStateNormal];
...
- (void)modeButtonPressed:(ModernButton *)sender
{
    ...
    if (editingHighlights != _editingHighlights)
    {
        ...
        TintToolValue *value = [(TintToolValue *)self.value copy];
...
- (void)swatchPressed:(PhotoEditorTintSwatchView *)sender
{
    TintToolValue *value = [(TintToolValue *)self.value copy];
    
    for (PhotoEditorTintSwatchView *swatchView in _swatchViews)
    {
...
- (bool)buttonPressed:(bool)cancelButton
{
    if (_editingIntensity)
    {
        ...
        __weak PhotoEditorTintToolView *weakSelf = self;
        [self setIntensitySliderHidden:true animated:true completion:^
        {
            __strong PhotoEditorTintToolView *strongSelf = weakSelf;
            if (strongSelf == nil)
                return;
...
- (void)sliderValueChanged:(PhotoEditorSliderView *)sender
{
    ...
    NSInteger newValue = (NSInteger)(floor(sender.value));
    if (_editingHighlights)
...
{
    if (![value isKindOfClass:[TintToolValue class]])
        return;
    
    _value = value;
    
    TintToolValue *tintValue = (TintToolValue *)value;
...
- (void)layoutSubviews
{
    ...
    {
        CGFloat leftEdge = 30;
        CGFloat spacing = (self.frame.size.width - leftEdge * 2 - PhotoEditorTintSwatchSize * _swatchViews.count) / (_swatchViews.count - 1);
        NSInteger i = 0;
    ...
    {
        CGFloat topEdge = 30;
        CGFloat spacing = (self.frame.size.height - 30 * 2 - PhotoEditorTintSwatchSize * _swatchViews.count) / (_swatchViews.count - 1);
        ...
        {
            swatchOffset = self.frame.size.width - 51 - PhotoEditorTintSwatchSize;
