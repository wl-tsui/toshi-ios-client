...
NSString *const MediaGroupCellKind = @"MediaGroupCellKind";
const CGFloat MediaGroupCellHeight = 86.0f;

@interface MediaGroupImageView : ImageView

@end

@implementation MediaGroupImageView
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self != nil)
    {
        ...
        ImageView *imageView2 = [[MediaGroupImageView alloc] initWithFrame:CGRectMake(12.0f, 7.0f, 61.0f, 61.0f)];
        ...
        ImageView *imageView1 = [[MediaGroupImageView alloc] initWithFrame:CGRectMake(10.0f, 9.0f, 65.0f, 65.0f)];
        ...
        ImageView *imageView0 = [[MediaGroupImageView alloc] initWithFrame:CGRectMake(8.0f, 11.0f, 69.0f, 69.0f)];
        ...
        for (ImageView *view in _imageViews)
...
- (void)_addBorderViewForImageView:(ImageView *)imageView
{
    static UIImage *borderImage = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^
...
- (void)prepareForReuse
{
    ...
    for (ImageView *imageView in _imageViews)
        [imageView reset];
}
...
- (void)configureThumbnailsWithAssets:(NSArray *)assets
{
    if (assets.count > 0)
    {
        for (NSUInteger i = 0; i < _imageViews.count; i++)
        {
            ...
            if (i < assets.count)
            {
                ...
                [imageView setSignal:[MediaAssetImageSignals imageForAsset:assets[i]
                                                                   imageType:MediaAssetImageTypeThumbnail
    ...
    else
    {
        ...
        [(ImageView *)_imageViews.firstObject setImage:[UIImage imageNamed:@"ModernMediaEmptyAlbumIcon"]];
    }
}
