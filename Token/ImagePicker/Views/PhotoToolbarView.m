...
- (instancetype)initWithBackButtonTitle:(NSString *)backButtonTitle doneButtonTitle:(NSString *)doneButtonTitle accentedDone:(bool)accentedDone solidBackground:(bool)solidBackground
{
    ...
    if (self != nil)
    {
        ...
        _backgroundView.backgroundColor = (solidBackground ? [PhotoEditorInterfaceAssets toolbarBackgroundColor] : [PhotoEditorInterfaceAssets toolbarTransparentBackgroundColor]);
        ...
        _doneButton = [[ModernButton alloc] initWithFrame:CGRectMake(0, 0, 40, 44)];
        _doneButton.exclusiveTouch = true;
        [_doneButton setTitle:doneButtonTitle forState:UIControlStateNormal];
        [_doneButton setTitleColor:(accentedDone ? [PhotoEditorInterfaceAssets accentColor] : [UIColor whiteColor])];
...
- (PhotoEditorButton *)createButtonForTab:(PhotoEditorTab)editorTab
{
    PhotoEditorButton *button = [[PhotoEditorButton alloc] initWithFrame:CGRectMake(0, 0, 28, 28)];
    ...
    switch (editorTab)
    {
        case PhotoEditorCaptionTab:
            button.iconImage = [PhotoEditorInterfaceAssets captionIcon];
            button.dontHighlightOnSelection = true;
            break;

        case PhotoEditorCropTab:
            button.iconImage = [PhotoEditorInterfaceAssets cropIcon];
            break;

        case PhotoEditorToolsTab:
            button.iconImage = [PhotoEditorInterfaceAssets toolsIcon];
            break;

        case PhotoEditorRotateTab:
            button.iconImage = [PhotoEditorInterfaceAssets rotateIcon];
            button.dontHighlightOnSelection = true;
            break;
            
        case PhotoEditorPaintTab:
            button.iconImage = [PhotoEditorInterfaceAssets paintIcon];
            break;
            
        case PhotoEditorStickerTab:
            button.iconImage = [PhotoEditorInterfaceAssets stickerIcon];
            button.dontHighlightOnSelection = true;
            break;
            
        case PhotoEditorTextTab:
            button.iconImage = [PhotoEditorInterfaceAssets textIcon];
            button.dontHighlightOnSelection = true;
            break;
            
        case PhotoEditorGifTab:
            [button setIconImage:[PhotoEditorInterfaceAssets gifIcon] activeIconImage:[PhotoEditorInterfaceAssets gifActiveIcon]];
            button.dontHighlightOnSelection = true;
            break;
            
        case PhotoEditorQualityTab:
            button.iconImage = [PhotoEditorInterfaceAssets qualityIconForPreset:MediaVideoConversionPresetCompressedMedium];
            button.dontHighlightOnSelection = true;
            break;
...
- (void)setToolbarTabs:(PhotoEditorTab)tabs animated:(bool)animated
{
    ...
    if (animated && _currentTabs != PhotoEditorNoneTab)
    {
    ...
    if (_currentTabs & PhotoEditorCropTab)
        [_buttonsWrapperView addSubview:[self createButtonForTab:PhotoEditorCropTab]];
    if (_currentTabs & PhotoEditorStickerTab)
        [_buttonsWrapperView addSubview:[self createButtonForTab:PhotoEditorStickerTab]];
    if (_currentTabs & PhotoEditorPaintTab)
        [_buttonsWrapperView addSubview:[self createButtonForTab:PhotoEditorPaintTab]];
    if (_currentTabs & PhotoEditorTextTab)
        [_buttonsWrapperView addSubview:[self createButtonForTab:PhotoEditorTextTab]];
    if (_currentTabs & PhotoEditorToolsTab)
        [_buttonsWrapperView addSubview:[self createButtonForTab:PhotoEditorToolsTab]];
    if (_currentTabs & PhotoEditorRotateTab)
        [_buttonsWrapperView addSubview:[self createButtonForTab:PhotoEditorRotateTab]];
    if (_currentTabs & PhotoEditorGifTab)
        [_buttonsWrapperView addSubview:[self createButtonForTab:PhotoEditorGifTab]];
    if (_currentTabs & PhotoEditorQualityTab)
        [_buttonsWrapperView addSubview:[self createButtonForTab:PhotoEditorQualityTab]];
...
- (void)tabButtonPressed:(PhotoEditorButton *)sender
{
...
{
    for (PhotoEditorButton *button in _buttonsWrapperView.subviews)
...
- (void)layoutSubviews
{
    ...
    else
    {
        ...
        {
            UIView *button = buttons.firstObject;
            button.frame = CGRectMake((self.frame.size.width - button.frame.size.width) / 2, floor((self.frame.size.height - button.frame.size.height) / 2), button.frame.size.width, button.frame.size.height);
        }
        else if (buttons.count == 2)
        {
            ...
            topButton.frame = CGRectMake((self.frame.size.width - topButton.frame.size.width) / 2, floor(self.frame.size.height / 5 * 2 - 10 - topButton.frame.size.height / 2), topButton.frame.size.width, topButton.frame.size.height);
        ...
        else if (buttons.count == 3)
        {
            ...
            topButton.frame = CGRectMake((self.frame.size.width - topButton.frame.size.width) / 2, floor(self.frame.size.height / 6 * 2 - 10 - topButton.frame.size.height / 2), topButton.frame.size.width, topButton.frame.size.height);
            centerButton.frame = CGRectMake((self.frame.size.width - centerButton.frame.size.width) / 2, floor((self.frame.size.height - centerButton.frame.size.height) / 2), centerButton.frame.size.width, centerButton.frame.size.height);
            bottomButton.frame = CGRectMake((self.frame.size.width - bottomButton.frame.size.width) / 2, CGCeil(self.frame.size.height - topButton.frame.origin.y - bottomButton.frame.size.height), bottomButton.frame.size.width, bottomButton.frame.size.height);
        }
