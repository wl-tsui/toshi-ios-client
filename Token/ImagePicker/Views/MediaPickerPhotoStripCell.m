...
NSString *const MediaPickerPhotoStripCellKind = @"PhotoStripCell";

@interface MediaPickerPhotoStripCell ()
{
...
@implementation MediaPickerPhotoStripCell
...
- (void)setItem:(NSObject *)item signal:(SSignal *)signal
{
    ...
    if (self.selectionContext != nil)
    {
        ...
        [self setChecked:[self.selectionContext isItemSelected:(id<MediaSelectableItem>)item] animated:false];
        __weak MediaPickerPhotoStripCell *weakSelf = self;
        [_itemSelectedDisposable setDisposable:[[self.selectionContext itemInformativeSelectedSignal:(id<MediaSelectableItem>)item] startWithNext:^(MediaSelectionChange *next)
        {
            __strong MediaPickerPhotoStripCell *strongSelf = weakSelf;
            if (strongSelf == nil)
                return;
            
            if (![next.sender isKindOfClass:[MediaPickerGallerySelectedItemsModel class]])
...
- (void)checkButtonPressed
{
    ...
    if (self.itemSelected != nil)
        self.itemSelected((id<MediaSelectableItem>)_item, _checkButton.selected, _checkButton);
}
