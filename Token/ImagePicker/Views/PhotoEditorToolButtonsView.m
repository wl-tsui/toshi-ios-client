...
#import "ModernButtonView.h" // HAVE
#import "Common.h"

const CGFloat PhotoEditorToolButtonsViewSize = 53;

@implementation PhotoEditorToolButtonsView
{
    UIView *_backgroundView;
    UIView *_stripeView;
    UIView *_topStripeView;
    
    ModernButtonView *_cancelButton;
    ModernButtonView *_confirmButton;
...
- (instancetype)initWithCancelButton:(NSString *)cancelButton doneButton:(NSString *)doneButton
{
    ...
    if (self != nil)
    {
        ...
        _cancelButton = [[ModernButtonView alloc] initWithFrame:CGRectZero];
        ...
        _confirmButton = [[ModernButtonView alloc] initWithFrame:CGRectZero];
...
- (void)layoutSubviews
{
    ...
    if (self.frame.size.width > self.frame.size.height)
    {
        ...
        _cancelButton.frame = CGRectMake(0, 0, floor(self.frame.size.width / 2), self.frame.size.height);
        _confirmButton.frame = CGRectMake(floor(self.frame.size.width / 2), 0, floor(self.frame.size.width / 2), self.frame.size.height);
        _cancelButton.titleLabel.font = TGSystemFontOfSize(17);
        _confirmButton.titleLabel.font = TGSystemFontOfSize(17);
    }
    else
...
- (void)calculateLandscapeSizeForPossibleButtonTitles:(NSArray *)possibleButtonTitles
{
    ...
    _landscapeSize = MAX(maxWidth, PhotoEditorToolButtonsViewSize);
}
