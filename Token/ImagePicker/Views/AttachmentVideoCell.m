...
NSString *const AttachmentVideoCellIdentifier = @"AttachmentVideoCell";

@interface AttachmentVideoCell () <ModernGalleryTransitionView>
{
...
@implementation AttachmentVideoCell
...
- (void)setAsset:(MediaAsset *)asset signal:(SSignal *)signal
{
    ...
    if (asset.subtypes & MediaAssetSubtypeVideoTimelapse)
        _iconView.image = [UIImage imageNamed:@"ModernMediaItemTimelapseIcon"];
    else if (asset.subtypes & MediaAssetSubtypeVideoHighFrameRate)
    ...
    __weak AttachmentVideoCell *weakSelf = self;
    [_adjustmentsDisposable setDisposable:[adjustmentsSignal startWithNext:^(VideoEditAdjustments *next)
    {
        __strong AttachmentVideoCell *strongSelf = weakSelf;
        if (strongSelf == nil)
            return;
        
        if ([next isKindOfClass:[VideoEditAdjustments class]])
...
- (void)_layoutImageForOriginalSize:(CGSize)originalSize cropRect:(CGRect)cropRect cropOrientation:(UIImageOrientation)cropOrientation
{
    ...
    CGSize scaledSize = ScaleToFillSize(cropRect.size, self.frame.size);
    ...
    CGPoint centerOffset = PaintSubtractPoints(centerPoint, [self fittedCropCenterRect:cropRect scale:scale]);

    self.imageView.bounds = CGRectMake(0, 0, fittedOriginalSize.width, fittedOriginalSize.height);
    self.imageView.center = PaintAddPoints(PaintCenterOfRect(self.bounds), centerOffset);
...
- (UIImage *)transitionImageSquared
{
    ...
    [[UIBezierPath bezierPathWithRoundedRect:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height) cornerRadius:AttachmentMenuCellCornerRadius] addClip];
    ...
    VideoEditAdjustments *adjustments = (VideoEditAdjustments *)[self.editingContext adjustmentsForItem:self.asset];
    ...
    CGSize fillSize = ScaleToFillSize(image.size, self.bounds.size);
    ...
    {
        CGContextConcatCTM(context, VideoCropTransformForOrientation(cropOrientation, self.frame.size, false));
...
- (UIImage *)transitionImage
{
    ...
    CGSize fillSize = ScaleToFillSize(cropRect.size, self.bounds.size);
    UIImage *croppedImage = PhotoEditorVideoCrop(sourceImage, nil, cropOrientation, 0, cropRect, false, fillSize, originalSize, true, true);
