...
@interface ModernGalleryVideoFooterView ()
{
    ModernButton *_playPauseButton;
}

@end

@implementation ModernGalleryVideoFooterView
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self != nil)
    {
        _playPauseButton = [[ModernButton alloc] init];
...
- (UIImage *)pauseImage
{
    ...
    dispatch_once(&onceToken, ^
    {
        ...
        CGContextSetFillColorWithColor(context, [UIColor whiteColor].CGColor);
        CGContextFillRect(context, CGRectMake(floor((21.0f - spacing - width * 2.0f) / 2.0f), spacingTop, width, 22.5f - spacingTop * 2.0f));
        CGContextFillRect(context, CGRectMake(floor((21.0f - spacing - width * 2.0f) / 2.0f) + width + spacing, spacingTop, width, 22.5f - spacingTop * 2.0f));
...
- (void)layoutSubviews
{
    ...
    _playPauseButton.frame = (CGRect){{floor((self.frame.size.width - buttonSize.width) / 2.0f), floor((self.frame.size.height - buttonSize.height) / 2.0f)}, buttonSize};
}
