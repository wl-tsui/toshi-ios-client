...
@interface HighlightsTool ()
{
    PhotoProcessPassParameter *_parameter;
}
@end

@implementation HighlightsTool
...
- (NSString *)title
{
    return TGLocalized(@"HighlightsTool");
}
...
- (NSArray *)parameters
{
    if (!_parameters)
    {
        _parameter = [PhotoProcessPassParameter parameterWithName:@"highlights" type:@"lowp float"];
        _parameters = @[ [PhotoProcessPassParameter constWithName:@"hsLuminanceWeighting" type:@"mediump vec3" value:@"vec3(0.3, 0.3, 0.3)"],
                         _parameter ];
    }
