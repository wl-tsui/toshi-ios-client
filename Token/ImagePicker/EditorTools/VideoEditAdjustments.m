...
const NSTimeInterval VideoEditMinimumTrimmableDuration = 3.0;
const NSTimeInterval VideoEditMaximumGifDuration = 30.5;

@implementation VideoEditAdjustments
...
{
    VideoEditAdjustments *adjustments = [[[self class] alloc] init];
...
+ (instancetype)editAdjustmentsWithDictionary:(NSDictionary *)dictionary
{
    ...
    if (dictionary[@"paintingImagePath"])
        adjustments->_paintingData = [PaintingData dataWithPaintingImagePath:dictionary[@"paintingImagePath"]];
    if (dictionary[@"sendAsGif"])
        adjustments->_sendAsGif = [dictionary[@"sendAsGif"] boolValue];
    if (dictionary[@"preset"])
        adjustments->_preset = (MediaVideoConversionPreset)[dictionary[@"preset"] integerValue];
...
- (NSDictionary *)dictionary
{
    ...
    if (self.preset != MediaVideoConversionPresetCompressedDefault)
...
{
    return ![self cropAppliedForAvatar:forAvatar] && ![self hasPainting] && !_sendAsGif && _preset == MediaVideoConversionPresetCompressedDefault;
}
...
{
    return (_CGRectEqualToRectWithEpsilon(self.cropRect, adjustments.cropRect, [self _cropRectEpsilon])) && self.cropOrientation == ((VideoEditAdjustments *)adjustments).cropOrientation;
}
...
- (BOOL)isEqual:(id)object
{
    ...
    VideoEditAdjustments *adjustments = (VideoEditAdjustments *)object;
