...
@implementation MediaAssetModernImageSignals
...
+ (SSignal *)_imageForAsset:(MediaAsset *)asset imageType:(MediaAssetImageType)imageType size:(CGSize)size allowNetworkAccess:(bool)allowNetworkAccess suppressNetworkError:(bool)suppressNetworkError
{    
    CGSize imageSize = size;
    if (imageType == MediaAssetImageTypeFullSize)
        imageSize = PHImageManagerMaximumSize;
    
    bool isScreenImage = (imageType == MediaAssetImageTypeScreen || imageType == MediaAssetImageTypeFastScreen);
    
    PHImageRequestOptions *options = [MediaAssetModernImageSignals _optionsForAssetImageType:imageType];
    
    PHImageContentMode contentMode = PHImageContentModeAspectFill;
    if (isScreenImage)
        contentMode = PHImageContentModeAspectFit;
    
    if ([asset representsBurst] && (isScreenImage || imageType == MediaAssetImageTypeFullSize))
    {
        SSignal *signal = [[[self imageDataForAsset:asset] filter:^bool(id value)
        {
            return [value isKindOfClass:[MediaAssetImageData class]];
        }] map:^UIImage *(MediaAssetImageData *data)
        {
            ...
            if (imageType != MediaAssetImageTypeFullSize)
            {
                ...
                image = ScaleImageToPixelSize(image, fittedSize);
            }
            
            return image;
        }];
        
        if (imageType == MediaAssetImageTypeFastScreen)
            signal = [[[self imageForAsset:asset imageType:MediaAssetImageTypeAspectRatioThumbnail size:CGSizeMake(128, 128) allowNetworkAccess:allowNetworkAccess] take:1] then:signal];
        
        return signal;
    }
    else if (asset.isVideo && asset.subtypes & MediaAssetSubtypeVideoHighFrameRate && isScreenImage)
    {
    ...
    else
    {
        SSignal *(^requestImageSignal)(bool) = ^SSignal *(bool networkAccessAllowed)
        {
            return [[SSignal alloc] initWithGenerator:^id<SDisposable>(SSubscriber *subscriber)
            {
                ...
                {
                    if (imageType == MediaAssetImageTypeFastScreen)
                        requestOptions = [MediaAssetModernImageSignals _optionsForAssetImageType:MediaAssetImageTypeScreen];
                    else
                        requestOptions = [options copy];

                    requestOptions.networkAccessAllowed = true;
                    
                    if (isScreenImage || imageType == MediaAssetImageTypeFullSize)
                    {
        ...
        if (allowNetworkAccess && !suppressNetworkError)
        {
            return [requestImageSignal(false) catch:^SSignal *(id error)
            {
                ...
                {
                    return [[[[self _imageForAsset:asset imageType:MediaAssetImageTypeAspectRatioThumbnail size:PhotoThumbnailSizeForCurrentScreen() allowNetworkAccess:true suppressNetworkError:true] filter:^bool(id value)
                    {
...
+ (SSignal *)imageDataForAsset:(MediaAsset *)asset allowNetworkAccess:(bool)allowNetworkAccess
{
    SSignal *(^requestDataSignal)(bool) = ^SSignal *(bool networkAccessAllowed)
    {
        return [[SSignal alloc] initWithGenerator:^id<SDisposable>(SSubscriber *subscriber)
        {
            PHImageRequestOptions *options = [MediaAssetModernImageSignals _optionsForAssetImageType:MediaAssetImageTypeFullSize];
            if (networkAccessAllowed)
            {
            ...
            PHImageRequestID token = [[self imageManager] requestImageDataForAsset:asset.backingAsset options:options resultHandler:^(NSData *imageData, NSString *dataUTI, __unused UIImageOrientation orientation, NSDictionary *info)
            {
                ...
                MediaAssetImageData *data = [[MediaAssetImageData alloc] init];
...
+ (SSignal *)fileAttributesForAsset:(MediaAsset *)asset
{
    SSignal *attributesSignal = [[SSignal alloc] initWithGenerator:^id<SDisposable>(SSubscriber *subscriber)
    {
        PHImageRequestID token = [[PHImageManager defaultManager] requestImageDataForAsset:asset.backingAsset options:nil resultHandler:^(NSData *data, NSString *dataUTI, __unused UIImageOrientation orientation, NSDictionary *info)
        {
            ...
            MediaAssetImageFileAttributes *attributes = [[MediaAssetImageFileAttributes alloc] init];
...
+ (PHImageRequestOptions *)_optionsForAssetImageType:(MediaAssetImageType)imageType
{
    PHImageRequestOptions *options = [PHImageRequestOptions new];
    
    switch (imageType)
    {
        case MediaAssetImageTypeFastLargeThumbnail:
            options.deliveryMode = PHImageRequestOptionsDeliveryModeOpportunistic;
            options.resizeMode = PHImageRequestOptionsResizeModeFast;
            break;
            
        case MediaAssetImageTypeLargeThumbnail:
            options.deliveryMode = PHImageRequestOptionsDeliveryModeHighQualityFormat;
            options.resizeMode = PHImageRequestOptionsResizeModeFast;
            break;
            
        case MediaAssetImageTypeAspectRatioThumbnail:
            options.deliveryMode = PHImageRequestOptionsDeliveryModeHighQualityFormat;
            break;
            
        case MediaAssetImageTypeScreen:
            options.deliveryMode = PHImageRequestOptionsDeliveryModeHighQualityFormat;
            options.resizeMode = PHImageRequestOptionsResizeModeExact;
            break;
            
        case MediaAssetImageTypeFastScreen:
            options.deliveryMode = PHImageRequestOptionsDeliveryModeOpportunistic;
            options.resizeMode = PHImageRequestOptionsResizeModeExact;
            break;
            
        case MediaAssetImageTypeFullSize:
            options.deliveryMode = PHImageRequestOptionsDeliveryModeHighQualityFormat;
...
{
    if (asset.subtypes & MediaAssetSubtypeVideoHighFrameRate)
    {
