...
struct VideoQualityRecord
{
    int quality;
    int size;
    NSString *url;
    
    VideoQualityRecord(int quality_, NSString *url_, int size_) :
        quality(quality_), size(size_)
    {
        url = url_;
    }
    
    VideoQualityRecord(const VideoQualityRecord &other)
    {
        url = other.url;
    ...
    VideoQualityRecord & operator= (const VideoQualityRecord &other)
    {
        if (this != &other)
        {
            url = other.url;
...
@interface VideoInfo ()
{
    std::vector<VideoQualityRecord> _qualitySet;
}
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    self = [super init];
    if (self != nil) {
        _qualitySet.push_back(VideoQualityRecord(0, [aDecoder decodeObjectForKey:@"url"], [aDecoder decodeInt32ForKey:@"size"]));
    }
    return self;
}
...
{
    if (![object isKindOfClass:[VideoInfo class]])
        return false;
    
    VideoInfo *other = object;
...
{
    _qualitySet.push_back(VideoQualityRecord(quality, url, size));
}
...
- (NSString *)urlWithQuality:(int)quality actualQuality:(int *)actualQuality actualSize:(int *)actualSize
{
    ...
    for (std::vector<VideoQualityRecord>::iterator it = _qualitySet.begin(); it != _qualitySet.end(); it++)
    {
