#import "ModernButtonViewModel.h"
#import "Common.h"
#import "ModernButtonView.h"

@implementation ModernButtonViewModel

- (Class)viewClass
{
    return [ModernButtonView class];
}

- (void)_updateViewStateIdentifier
{
    self.viewStateIdentifier = [[NSString alloc] initWithFormat:@"ModernButtonView/%lx/%lx/%@/%lx/%lx", (long)_backgroundImage, (long)_highlightedBackgroundImage, _title, (long)_font, (long)_image];
}
...
- (void)bindViewToContainer:(UIView *)container viewStorage:(ModernViewStorage *)viewStorage
{
    ...
    ModernButtonView *view = (ModernButtonView *)[self boundView];
...
- (void)unbindView:(ModernViewStorage *)viewStorage {
    [(ModernButtonView *)self.boundView removeTarget:self action:@selector(buttonPressed) forControlEvents:UIControlEventTouchUpInside];
...
- (void)setTitle:(NSString *)title
{
    ...
    {
        [(ModernButtonView *)[self boundView] setTitle:_title];
    }
}
...
- (void)setImage:(UIImage *)image
{
    ...
    if ([self boundView] != nil)
        [(ModernButtonView *)[self boundView] setImage:_image];
}
...
- (void)setBackgroundImage:(UIImage *)backgroundImage
{
    ...
    if ([self boundView] != nil)
        [(ModernButtonView *)[self boundView] setBackgroundImage:_backgroundImage];
}
...
- (void)drawInContext:(CGContextRef)__unused context
{
    if (!self.skipDrawInContext && self.alpha > FLT_EPSILON && !self.hidden)
    {
        ...
        if (_image != nil)
        {
            ...
            [_image drawInRect:CGRectMake(floor((bounds.size.width - imageSize.width) / 2.0f), floor((bounds.size.height - imageSize.height) / 2.0f), imageSize.width, imageSize.height)];
        }
        if (_title.length != 0) {
            CGSize titleSize = [_title sizeWithFont:_font];
            [_title drawInRect:CGRectMake(floor(self.frame.size.width - titleSize.width), floor(self.frame.size.height - titleSize.height), titleSize.width, titleSize.height) withFont:_font];
        }
    }
}
...
- (void)setDisplayProgress:(bool)displayProgress animated:(bool)animated {
    ...
    [(ModernButtonView *)[self boundView] setDisplayProgress:displayProgress animated:animated];
}
