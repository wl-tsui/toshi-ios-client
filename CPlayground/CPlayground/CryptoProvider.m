...
+ (void)addDefaultProviderToContext:(signal_context *)context
{
    signal_crypto_provider provider = {
        .random_func = cc_random_generator,
        .hmac_sha256_init_func = cc_hmac_sha256_init,
        .hmac_sha256_update_func = cc_hmac_sha256_update,
        .hmac_sha256_final_func = cc_hmac_sha256_final,
        .hmac_sha256_cleanup_func = cc_hmac_sha256_cleanup,
        .sha512_digest_init_func = cc_sha512_digest_init,
        .sha512_digest_update_func = cc_sha512_digest_update,
        .sha512_digest_final_func = cc_sha512_digest_final,
        .sha512_digest_cleanup_func = cc_sha512_digest_cleanup,
        .encrypt_func = cc_encrypt,
        .decrypt_func = cc_decrypt,
        .user_data = 0 // TODO: Is this correct for an actual instance? 
    };
...
/*
 C methods ganked from the tests for CommonCrypto on libsignal-protocol-c:

 https://github.com/signalapp/libsignal-protocol-c/blob/master/tests/test_common_ccrypto.c
 */

int cc_random_generator(uint8_t *data, size_t len, void *user_data)
{
...
int cc_hmac_sha256_init(void **hmac_context, const uint8_t *key, size_t key_len, void *user_data)
{
...
int cc_hmac_sha256_update(void *hmac_context, const uint8_t *data, size_t data_len, void *user_data)
{
...
int cc_hmac_sha256_final(void *hmac_context, signal_buffer **output, void *user_data)
{
    CCHmacContext *ctx = hmac_context;
...
void cc_hmac_sha256_cleanup(void *hmac_context, void *user_data)
{
    if(hmac_context) {
        CCHmacContext *ctx = hmac_context;
        free(ctx);
...
int cc_sha512_digest_init(void **digest_context, void *user_data)
{
    int result = 0;
...
int cc_sha512_digest_update(void *digest_context, const uint8_t *data, size_t data_len, void *user_data)
{
...
int cc_sha512_digest_final(void *digest_context, signal_buffer **output, void *user_data)
{
    int result = 0;
...
void cc_sha512_digest_cleanup(void *digest_context, void *user_data)
{
    if(digest_context) {
        CC_SHA512_CTX *ctx = digest_context;
        free(ctx);
...
int cc_encrypt(signal_buffer **output,
                 int cipher,
...
int cc_decrypt(signal_buffer **output,
                 int cipher,
                 const uint8_t *key, size_t key_len,
                 const uint8_t *iv, size_t iv_len,
